...
function carousel_obj_swap() {
    ...
    if (index === total-1) {
        document.querySelector(".obj").children[0].innerHTML = "●";
    }
}
...
window.onload = function () {
    ...
    //캐러셀 슬라이드 자동이동
    var carousel_slide = function (slide, total) {
        index++;
        slide.style.transition = "transform 300ms";
        slide.style.transform = "translateX(" + (-100 * index) + "%)";
        carousel_obj_swap();
        if (index === total) {
            setTimeout(carousel_slide_reset(slide), 0);
        }
    };
    ...
    //캐러셀 화면전환 시작
    timerId = setTimeout(carousel(slide_reset), delay);
